...
#define CMP_MSC_OUT_EP                      MSC_EPOUT_ADDR      /* EP1 for BULK OUT */
#define CMP_MSC_IN_EP                       MSC_EPIN_ADDR       /* EP1 for BULK IN */
#define CMP_CDC_CMD_EP                      CDC_CMD_EP          /* EP2 for CDC commands */
#define CMP_CDC_OUT_EP                      CDC_OUT_EP          /* EP3 for data OUT */
#define CMP_CDC_IN_EP                       CDC_IN_EP           /* EP3 for data IN */
...
/** @defgroup USBD_CDC_Private_FunctionPrototypes
  * @{
  */
static uint8_t USBD_MSC_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx);//static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx);
static uint8_t  USBD_MSC_CDC_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx);//static uint8_t USBD_CDC_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx);
static uint8_t  USBD_MSC_CDC_Setup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req);//static uint8_t USBD_CDC_Setup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req);
static uint8_t  USBD_MSC_CDC_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum); //static uint8_t USBD_CDC_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum);
static uint8_t  USBD_MSC_CDC_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum); //static uint8_t USBD_CDC_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum);
static uint8_t  USBD_MSC_CDC_EP0_RxReady(USBD_HandleTypeDef *pdev); //static uint8_t USBD_CDC_EP0_RxReady(USBD_HandleTypeDef *pdev);
static uint8_t  *USBD_MSC_CDC_GetFSCfgDesc(uint16_t *length);  //static uint8_t *USBD_CDC_GetFSCfgDesc(uint16_t *length);
//static uint8_t *USBD_MSC_CDC_GetHSCfgDesc(uint16_t *length);
//static uint8_t *USBD_MSC_CDC_GetOtherSpeedCfgDesc(uint16_t *length);
//static uint8_t *USBD_MSC_CDC_GetOtherSpeedCfgDesc(uint16_t *length);
static uint8_t  *USBD_MSC_CDC_GetDeviceQualifierDesc(uint16_t *length);//uint8_t *USBD_CDC_GetDeviceQualifierDescriptor(uint16_t *length);
...
static uint8_t USBD_MSC_CDC_CfgDesc[USB_MSC_CDC_CONFIG_DESC_SIZ] =
{
	...
	CMP_MSC_IN_EP,   /*Endpoint address (IN, address 1) */
	...
	0x07,   /*Endpoint descriptor length = 7 */
	0x05,   /*Endpoint descriptor type */
	CMP_MSC_OUT_EP,   /*Endpoint address (OUT, address 1) */
	0x02,   /*Bulk endpoint type */
	LOBYTE(USB_MAX_PACKET_SIZE),
	HIBYTE(USB_MAX_PACKET_SIZE),
	0x00,     /*Polling interval in milliseconds*/
	...
	CMP_CDC_CMD_EP,                    /* bEndpointAddress */
	...
	0x07,   /* bLength: Endpoint Descriptor size */
	USB_DESC_TYPE_ENDPOINT,        /* bDescriptorType: Endpoint */
	CMP_CDC_OUT_EP,                    /* bEndpointAddress */
	...
	0x07,   /* bLength: Endpoint Descriptor size */
	USB_DESC_TYPE_ENDPOINT,        /* bDescriptorType: Endpoint */
	CMP_CDC_IN_EP,                     /* bEndpointAddress */
	0x02,                          /* bmAttributes: Bulk */
	LOBYTE(CDC_DATA_PACKET_SIZE),  /* wMaxPacketSize: */
	HIBYTE(CDC_DATA_PACKET_SIZE),
	0x00,                          /* bInterval */
...
static uint8_t  USBD_MSC_CDC_Setup (USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	...
	if(((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_INTERFACE && req->wIndex == MSC_INTERFACE_IDX) ||
		((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_ENDPOINT && ((req->wIndex & 0x7F) == CMP_MSC_OUT_EP)))
	{
